...
class UIdirectives:
    'list the oirectives that can be passed to the agents '

    # Originally these appeared in the UI. Now they are only used in the code so this indirections
    # is a pointelss complication. We should use the Host agent ones directly.
    EXTRACTAUDIO = BVMediaProcAgent.EXTRACTAUDIO
...
class UIMapper (object):
    ...
    map = {
            ...
            TalkKeys.STATUS: '3',
            UPLDPATTERN: '4',
            TalkKeys.Descriptor.DATE: '5',
            TalkKeys.Descriptor.TRAINER: '6',
            TalkKeys.Descriptor.CONTEXT: '7',
            TalkKeys.Descriptor.TITLE: '8',
            TalkKeys.Descriptor.LANGUAGE: '9',
            TalkKeys.Descriptor.TAGS: '10',
            TalkKeys.Descriptor.CATEGORY: '11',
            TalkKeys.Descriptor.DESC: '12',
            TalkKeys.Descriptor.ACCESS: '13',
            TalkKeys.Descriptor.EDITOR: '14',
            VIDEOSRC: '15',
            AUDIOSRC: '16',
            VIDEOTHUMB: '17',
            TRAINTHUMB:'18',
            TalkKeys.Descriptor.FNAME: '19',
            TalkKeys.Descriptor.SEODESC: '20',
            TalkKeys.Descriptor.SEOTAGS: '21',
            YOUTUBE: '22',
            VIMEO:'23',
            HWDVIDEO : '24',
            HWDAUDIO:'25',
            S3VIDEO: '26',
            S3AUDIO:'27',
            TalkKeys.Descriptor.EXCPTCOMP:'28'
        }
...
class DBgspread (BVOneClickDB) :
    ...
    def __valueToDate(self,v):
        ...
        return date
    def __valueToTagList(self,v): return v.split(UIdirectives.LISTDELIMITER)
    ...
    def __loadTalkDescriptor(self, desc, r):
        ...
        key = TalkKeys.Descriptor.DATE
        desc[key] = self.__valueToDate(r[UIMapper.map[key]])

        key = TalkKeys.Descriptor.TRAINER
        desc[key] = r[UIMapper.map[key]]

        key = TalkKeys.Descriptor.CONTEXT
        desc[key] = r[UIMapper.map[key]]

        key = TalkKeys.Descriptor.EDITOR
        desc[key] = r[UIMapper.map[key]]

        key = TalkKeys.Descriptor.LANGUAGE
        desc[key] = r[UIMapper.map[key]]

        key = TalkKeys.Descriptor.DESC
        desc[key] = r[UIMapper.map[key]]

        key = TalkKeys.Descriptor.TITLE
        desc[key] = r[UIMapper.map[key]]

        key = TalkKeys.Descriptor.SEODESC
        desc[key] = r[UIMapper.map[key]]

        key = TalkKeys.Descriptor.TAGS
        desc[key] = self.__valueToTagList(r[UIMapper.map[key]])

        key = TalkKeys.Descriptor.SEOTAGS
        desc[key] = self.__valueToTagList(r[UIMapper.map[key]])

        key = TalkKeys.Descriptor.CATEGORY
        desc[key] = r[UIMapper.map[key]]

        key = TalkKeys.Descriptor.ACCESS
        desc[key] = r[UIMapper.map[key]]

        key = TalkKeys.Descriptor.FNAME
        desc[key] = r[UIMapper.map[key]]

        key = TalkKeys.Descriptor.EXCPTCOMP
        desc[key] = r[UIMapper.map[key]]
    ...
    def __rowToTalk(self, r, rnum):
        ...
        action = self.__valueToAction(r[UIMapper.map[TalkKeys.ACTION]])
        if (action== BVTalk.SKIP or action == BVTalk.NOOP or action == '' ): return None
        ...
        # Id and  Status
        id = r[UIMapper.map[TalkKeys.ID]]
        status = self.__valueToStatus(r[UIMapper.map[TalkKeys.STATUS]])

        # DESCRIPTOR including calculated descriptions
        self.__loadTalkDescriptor(desc, r)

        #################### Talk creation #######################
        talk = BVTalk(id, desc, action, status)
        ...
        video = MediaSource(handle, TalkKeys.BVMediaTypes.VIDEO)
        videomedia = BVTalkMedia(video)
        ...
        if UIMapper.isActive(vdirectives) and (not status.isDone(key)):
            videoproc = MP4MediaProcAgent(key, vdirectives.split(','))
            videoproc.setmedia(videomedia)
        ...
        #cnum = UIMapper.cmap[ccode]
        handle = self.__valueToURL(r[ccode])
        audio = MediaSource(handle, TalkKeys.BVMediaTypes.AUDIO)
        audiomedia = BVTalkMedia(audio)
        ...
        if UIMapper.isActive(adirectives) and (not status.isDone(key)):
            audioproc = MP3ID3SMediaProcAgent(key, adirectives.split(','))
            audioproc.setmedia(audiomedia)
            #eventh = GSHEETMediaEventHandler(rnum, cnum)
        ...
        talk.subscribeToAllAgents(GSHEETMediaEventHandler(talk, rnum))
...
class GSHEETMediaEventHandler (BVMediaProcEventHandler, BVMediaHostEventHandler):
    ...
    def _updateStatusCell(self, agent, string):
        ...
        ccode = UIMapper.map[TalkKeys.STATUS]
        cnum = UIMapper.cmap[ccode]
...
class UIgspreadTalkHandler (BVTalkEventHandler):
    def __init__(self):
        return
    def handle(self, event):
        log.stdout( "received an event from talk "+ event.source +  " msg= " + event.message)
        return


class UIgspread (BVOneClickUI):

    def initialize (self, talkjobs):
        'talk jobs are now just talks'
        log.stdout("Initializing UI handler")

        # create a talk handler to subscribe to all talks
        h = UIgspreadTalkHandler()

        """ I decided to implemte events at agnts level directly
        # register an event handler in each of the talks
        for t in talkjobs:
            #t = tj.getTalk()
            t.subscribe(h)
        """
