...
###############  Model Factory: #################################################
# I struggled with Python scopes here. try to make it works but not quite got the funcion used as objects in python.
MDhaFactoryMethod = None
MDaudioPaFactoryMethod = None
MDvideoPaFactoryMethod = None


def setMDFactoryMethods(ha, audioPa, videopa):
    global MDhaFactoryMethod, MDaudioPaFactoryMethod, MDvideoPaFactoryMethod
    MDhaFactoryMethod = ha
    MDaudioPaFactoryMethod = audioPa
...
class MDFactory(object):
    ...
    def createHostAgent(self, akey, videothumb, directives):
        global MDhaFactoryMethod
        return MDhaFactoryMethod(akey, videothumb, directives)

    def createAudioSrcProcAgent(self, key, directives):
        global MDaudioPaFactoryMethod
        return MDaudioPaFactoryMethod(key, directives)

    def createVideoSrcProcAgent(self, key, directives):
...
class BVMediumHostAgent(BVAgentInterface):
    ...
    def _initSubclass(self, key, thumb, ds):
        pass
    ...
    def getTalk(self):
        return self.getMedia().getTalk()

    def getDirectives(self):
        return self.__directives  #list
...
class OcuUploadPattern(object):
    ...
    def getHostVideoAgentsKeys(self):
        res = []
        for key, value in self.__vpattern.iteritems():
            agentkey = key
    ...
    def getDirectives(self, agentKey):
        if agentKey in self.__apattern:
            return self.__apattern[agentKey]
        else:
            return self.__vpattern[agentKey]

    def getHostAudioAgentsKeys(self):
        res = []
        for key, value in self.__apattern.iteritems():
            agentkey = key
            if (agentkey != BVMediumProcAgent.VIDEOSOURCE) and (agentkey != BVMediumProcAgent.AUDIOSOURCE):
...
class OcuTalkJobFactory(object):
    ...
    def createTalkJob(action, status, talkdesc, videosrc, videothumb, uploadptrn, audiosrc=None):
        ...
        hosts = uploadptrn.getHostVideoAgentsKeys()
        ...
        hosts = uploadptrn.getHostAudioAgentsKeys()
        for hagentkey in hosts:
            if not status.isDone(hagentkey):
                # creates the medium host
                host = mdFactory.createHostAgent(hagentkey, videothumb, uploadptrn.getDirectives(hagentkey))
...
class BVTalkJob(BVJobInterface, BVTalkInterface):
    ...
    def getVideoTitle(self):
        return "%s_Vtitle" % (self.getID())  #mockup

    def getAudioTitle(self):
        return "%s_Atitle" % (self.getID())  #mockup

    def getFileNamePrefix(self):
        return "%s_Fname" % (self.getID())  #mockup

    def getBasicDescriptor(self):
